...
module.exports = function(grunt){
    ...
    function setTargetConfigValues(target){
        if(target === 'beta'){
            ...
            nconf.set('BASE_URL', 'http://assets-beta.extension.fm');
            nconf.set('API_URL', 'http://ex.fm');
            nconf.set('WEINRE', 'exfm-beta');
            nconf.set('S3_BUCKET', 'assets-beta.extension.fm');
            nconf.set('SOCKETIO', false);
            nconf.set('MINIFY', true);
            nconf.set('COMPRESS', true);
            nconf.set('FB_APP_ID', '234391086577435');
        }
        if(target === 'prod'){
            nconf.set('NODE_ENV', 'prod');
            nconf.set('BASE_URL', 'http://assets.extension.fm');
            nconf.set('API_URL', 'http://ex.fm');
            nconf.set('WEINRE', false);
            nconf.set('S3_BUCKET', 'assets.extension.fm');
            nconf.set('SOCKETIO', false);
            nconf.set('MINIFY', true);
            nconf.set('COMPRESS', true);
            nconf.set('FB_APP_ID', '112046155500548');
        }
    }
    ...
    grunt.registerTask('deploy.beta', function(){
        ...
        grunt.helper('deploy', 'beta').then(function(){
            done();
        });
    });
    ...
    grunt.registerTask('deploy.prod', function(){
        ...
        grunt.helper('deploy', 'prod').then(function(){
            done();
        });
    });
    ...
    grunt.registerHelper('deploy', function(target){
        var start = new Date();
        ...
        common.log('Building....');
        return grunt.helper('build').then(function(){
        //     return grunt.helper('smoke');
        // }).then(function(){
            ...
            return getLastModifiedTimes(allPaths).then(function(res){
                ...
                return when.all(paths.map(function(p){
                    return grunt.helper('fileInfo', nconf.get('ASSETS') + '/' + p);
                }));
    ...
    grunt.registerHelper('smoke', function(){
        var d = when.defer();
        common.log('Running smoke test. Please be patient...');
        return grunt.helper('startServer').then(function(port){
            var cmd = '/usr/local/bin/phantomjs ' + nconf.get('SMOKE_TEST') + ' http://local-dev.ex.fm:'+port+'/#bootstrap=http://local-dev.ex.fm:'+port+'/web/bootstrap.js';
            common.log(cmd);
        ...
        // @todo (lucas) Run test suite here.
            ...
            return grunt.helper('stopServer');
        });
    });
    ...
    //             aws.s3.deleteObject(nconf.get('S3_BUCKET'), item).then(function(){
    //                 complete();
    //             });
    //         }, 1);

    //     aws.onConnected(function(){
    //         aws.s3.listKeys(nconf.get('S3_BUCKET')).then(function(keys){
