...
module.exports = function deployopsworksStack(ctx, done){
    ...
    module.exports.stack(ctx, function(err, stack){
        ...
        async.waterfall([
            ...
            function watForDeploy(data, callback){
                module.exports.waitForDeploy(data.DeploymentId, ctx, done);
            }
        ], done);
    });
};

module.exports.waitForDeploy = function(deploymentId, ctx, done){
    var ow = new aws.OpsWorks();

    ow.describeDeployments({'DeploymentIds': [deploymentId]}, function(err, data){
        if(err){
            return done(err);
        }
        ctx.lastDeploy = data.Deployments[0];
        if(data.Deployments[0].Status === 'running'){
            debug('deploy still running.  will check again in 10s');
            return setTimeout(function(){
                module.exports.waitForDeploy(deploymentId, ctx, done);
            }, 10000);
        }
        debug('deploy finished with status ' + data.Deployments[0].Status);
        if(data.Deployments[0].Status === 'failed'){
            return done(new Error('Deploy failed.  Check the AWS console.'));
        }
        done();
    });
};
