...
RecipeInTheOven.prototype.runTask = function(taskName, done, isChild){
    ...
    var debug = require('debug')('mott:recipe'),
        spaces = (isChild) ? '    ' : '',
        self = this,
        tasks = [];

    debug(spaces + 'start task `'+taskName+'`');
        ...
        task.steps.map(function(step){
            ...
            if(!func && step !== taskName){ // Its another task.
                ...
                tasks.push.apply(tasks, func.map(function(_){
                    return function(cb){
                        debug(spaces + '    ' + step, 'running');
                        _(self.ctx, function(err){
                            debug(spaces + '    ' + step, 'completed');
                            cb(err);
                        });
                    };
                }));
    ...
    async.series(tasks, function(err){
        debug(spaces + 'finshed task `' + taskName + '`');
        done(err);
    });
};
...
Recipe.prototype.configure = function(metadata, config){
    metadata = util._extend(this.providesMetadata, metadata);
    this.config = util._extend(this.providesConfig, config);
...
Recipe.prototype.use = function(otherRecipes){
    ...
    otherRecipes.map(function(recipe){
        ...
        Object.keys(recipe.steps).map(function(stepName){
            recipe.steps[stepName].map(function(step){
