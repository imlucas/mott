...
module.exports = function build(platform){
    ...
    return when.all(platforms.map(function(platform){
        ...
        return when.all(platforms.map(function(platform){
            return module.exports.buildManifest(platform);
        }));
    }).then(function(){
...
module.exports.buildManifest = function(platform){
    var assets = path.resolve(nconf.get('ASSETS')),
        d = when.defer(),
        template = [
            'CACHE MANIFEST',
            ...
            '# PLATFORM: ' + platform,
            '# Auto generated by mott.  Do not check in.',
            'CACHE:',
            '{{files}}',
            'NETWORK:',
            '*'
        ].join('\n'),
        pRegex = new RegExp(platform + '|common'),
    ...
    allPaths = findit.sync(nconf.get('ASSETS')).filter(function(p){
                ...
                p.indexOf('handlebars-environment.js') === -1 &&
                p.indexOf('bootstrap') === -1 &&
                p.indexOf('index.html') === -1 &&
                p.indexOf('.appcache') === -1 &&
                p.indexOf('.') > -1;
                if(!i){
                    return i;
                }
                ...
                return f.replace(nconf.get('ASSETS') + '/', '/').replace('/web/', '/');
            });

    common.writeFile(assets + '/' + platform + '/app.appcache',
        template.replace('{{files}}', allPaths.join('\n'))).then(d.resolve, d.reject);
